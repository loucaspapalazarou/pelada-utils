...
def kill_existing_pb():
    system = platform.system()
    if system == "Windows":
        # /F = force, /IM = image name
        subprocess.run(
            ["taskkill", "/F", "/IM", "pelada-backend.exe"],
...
def initialize_pocketbase():
    ...
    subprocess.run(
        [str(PB_EXEC), "superuser", "upsert", SUPERUSER_EMAIL, SUPERUSER_PASSWORD],
        cwd=PB_ROOT,
        check=True,
    )
    ...
    server_proc = subprocess.Popen(
        [str(PB_EXEC), "serve", "--dev", f"--http=0.0.0.0:{PB_PORT}"],
        cwd=PB_ROOT,
...
def approve_pending_members(auth: AuthModel):
    ...
    url = PB_API_COLLECTIONS.format(collection="teams", operation="records")
    headers = {"Authorization": f"Bearer {auth.token}"}
    params = {"filter": f"(captain='{auth.id}')"}
